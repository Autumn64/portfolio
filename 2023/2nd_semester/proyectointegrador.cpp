...
#include <vector> /*Utilizar un array est�tico de structs limitar�a la cantidad de estudiantes que pueden agregarse. Hicimos pruebas para usar
                    un array din�mico y funciona bien en C, pero est� literalmente roto en C++, por lo que la alternativa m�s sensata y eficiente
                    es usar un vector en donde los structs de cada estudiante se guarden en cada espacio de �ste.*/
using namespace std;
...
void menu(vector<Estudiante>& estudiantes) {
	...
	cin >> o;
	cin.ignore(1, '\n'); //cin deja un salto de linea en el flujo de entrada, lo cual crea problemas en el resto del c�digo.
	switch (o) {
		case '1':
			adds(estudiantes);
			break;
...
int main()
{
	...
	do{
		menu(estudiantes);
	} while (true); //El programa regresar� indefinidamente al men� principal hasta que se use la opci�n 6.
	return 0;
}
